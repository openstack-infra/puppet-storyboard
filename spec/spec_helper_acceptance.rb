...
RSpec.configure do |c|
  ...
  c.before :suite do
    ...
    hosts.each do |host|
      ...
      on host, puppet('module','install','puppetlabs-apt'), { :acceptable_exit_codes => [0,1] } #inherited from rabbitmq
      on host, puppet('module','install','puppetlabs-apache', '--version', '0.0.4'), { :acceptable_exit_codes => [0,1] } #inherited from rabbitmq
      ...
      on host, puppet('module','install','example42-puppi', '--version', '2.1.9'), { :acceptable_exit_codes => [0,1] }
      ...
      on host, "apt-get install -y git; git clone https://github.com/puppetlabs/puppetlabs-rabbitmq.git /etc/puppet/modules/rabbitmq"
    end
  end
end
